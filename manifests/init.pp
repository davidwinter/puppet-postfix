class postfix {
	...
	exec { 'postfix set mydestination':
		command => "sed -i.bak 's/mydestination = .*/mydestination = localhost/' /etc/postfix/main.cf",
	...
	exec { 'postfix set inet_interfaces':
		command => "sed -i.bak 's/inet_interfaces = .*/inet_interfaces = 127.0.0.1/' /etc/postfix/main.cf",
